...
namespace GMlib {
  ...
  public:
    ...
    // The scalingoperator, there is also a non-member scaling operator with
    // oposit order!!
    //-------------------------------------------------------------------------------------
  ...
  template <typename T>
  inline DVector<T> operator*(const double d, DVector<T> p) {
    return p *= d;
  }
  ...
  T_Stream& operator<<(T_Stream& out, const DVector<T>& v) {
    ...
    for (int i = 0; i < v.getDim(); i++) out << v(i) << GMseparator::element();
  ...
  T_Stream& operator>>(T_Stream& in, DVector<T>& v) {
    ...
    for (int i = 0; i < v.getDim(); i++) in >> v[i] >> es;
  ...
  inline T operator*(const DVector<K>& m, const DVector<T>& v)
  {
#ifdef DEBUG
    if (m.dim() != v.getDim())
      std::cerr << "Dimension error, dim1=" << m.getDim()
                << " ,dim2=" << v.getDim() << std::endl;
#endif
    T r(0.0);
  ...
  template <typename T, typename K>
  inline K operator^(const DVector<K>& m, const DVector<T>& v)
  {
#ifdef DEBUG
    if (m.dim() != v.getDim())
      std::cerr << "Dimension error, dim1=" << m.getDim()
