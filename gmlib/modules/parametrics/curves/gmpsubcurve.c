 ...
 */
  ...
  template <typename T>
  void PSubCurve<T>::updateMat() const {
     this->_mat = this->_matrix.template toType<T>();
  ...
  /*! Point<T,3>  PSubCurve<T>::getSamplePoint(int i, int j) const
  ...
  Point<T,3>  PSubCurve<T>::getSamplePoint(int i, int j) const {
     ...
     return (*(this->_sampler))[j].sample_val[i][0] + _c_init_pos;
  }



  /*! Vector<T,3>& PSubCurve<T>::getSampleDerivative(int i, int j, int d) const
  ...
  Vector<T,3>&  PSubCurve<T>::getSampleDerivative(int i, int j, int d) const {
    ...
    return (*(this->_sampler))[j].sample_val[i][d];
  }


  /*! int PSubCurve<T>::getSample_d(int i, int j) const
  ...
  int PSubCurve<T>::getSample_d(int i, int j) const {
    ...
    return (*(this->_sampler))[j].sample_val[i].getDim();
  }
