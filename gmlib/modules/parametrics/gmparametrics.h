 ...
 *************************************************************************/
    ...
    inline
    T_Stream& operator <<  (T_Stream &out, const VisuPar& v) {
        out << v.m << ", " << v.is << ", " << v.ie << std::endl;
        return out;
    }
    ...
    // Each element in the vector is for a given sampling point
    template <typename T>
...
//        for(unsigned int i=0; i<pre_bas.size(); i++) {
...
//    }
    ...
    template <typename T>
    inline
    int getIntervallInVector( const std::vector<T>& t ) {
        int j=0;
    ...
    template <typename T>
    inline
    int getIntervallInVector( const DVector<T>& t ) {
        int j=0;
