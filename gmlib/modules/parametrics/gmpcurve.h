...
namespace GMlib {
  ...
  public:
    ...
    virtual Point<T,n>           getSamplePoint( int i, int j) const;
    virtual Vector<T,n>&         getSampleDerivative( int i, int j, int d) const;
    virtual int                  getSample_d( int i, int j) const;
    ...
    void                         setLocalPreEval(bool local_pre = true) { _local_pre_eval = local_pre;}
    ...
    void                         setNoDer( int d );
    void                         setSurroundingSphere( const std::vector< DVector< Vector<T,n> > >& p ) const;
    ...
    void                         setLineWidth(double w) const;
    ...
    void                         setIndPreEval( const std::vector<int>& p) const {_ind_trans_pre = p;}
...
//    mutable std::vector<int>     _index_map;   //!< Map of indices
    ...
    mutable std::vector<int>     _ind_trans_pre;    // Mapping of indises for pre-evalueted values
    mutable bool                 _local_pre_eval;   // true/false if it is a local curve with index preevaluation
    ...
    virtual void                 computeSurroundingSphere( const std::vector<DVector<Vector<T,n>>>& p, Sphere<T,n>& s ) const;
    virtual void                 resample( std::vector<DVector<Vector<T,n>>>& p, Sphere<T,3>& s, const std::vector<T>& t, int d) const;
