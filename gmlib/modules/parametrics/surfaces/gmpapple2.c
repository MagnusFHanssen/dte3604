...
template <typename T>
  ...
  template <typename T>
  void PApple2<T>::computeSurroundingSphere( const DMatrix<DMatrix<Vector<T,3>>>& /*p*/, Sphere<T,3>& s ) const {

      s.resetPos(Point<T,3>(T(0), T(0), -1.68*_r));
      s.resetRadius(2.635*_r);
  }
