...
namespace GMlib {
  ...
  private:
    struct EditSet {
       Point<int,2>   ind;   //!< Index of a selector and thus a control point
       Vector<T,3>    dp;    //!< The distance the control point has been moved
       EditSet(Point<int,2> i, const Vector<T,3>& d): ind(i),dp(d){}
    };
...
//      void                       edit(int selector) override;
  ...
  protected:
      ...
      mutable std::vector<DMatrix<T>> _pre_u;      // Pre-evaluation of basis in u-direction
      mutable std::vector<DMatrix<T>> _pre_v;      // Pre-evaluation of basis in v-direction
  ...
  private:
      ...
      // Help functions
      void                       internalPreSample( std::vector< DMatrix< T > >& p, int m, int d, T scale, T start, T end );
      void                       multEval(const DMatrix<T>& bu, const DMatrix<T>& bv, int du, int dv) const;

      void                       comp(DMatrix<Vector<T,3>>& p, const DMatrix<T>& Bu, const DMatrix<T>& Bv, const Vector<T,3>& c, Point<int,2> k) const;
      Point<int,2>               mapIndex(int i);
  ...
  private:
      ...
      template <typename T_Stream>
      void prOut(T_Stream& out) const { out << "her er vi" << std::endl; out << _c << std::endl; }
