...
namespace GMlib {
  ...
  private:
    ...
    struct CpIndex : public DMatrix<Matrix<int,2,2>> {
  ...
  public:
      ...
      void                       sample( int m1, int m2, int d1 = 0, int d2 = 0 ) override;
  ...
  protected:
      ...
      mutable std::vector<PreBasis<T>>    _pre_basis_u; //!< Pre-evaluated basis functions for each partition in u-direction
      mutable std::vector<PreBasis<T>>    _pre_basis_v; //!< Pre-evaluated basis functions for each partition in v-direction
      mutable DVector<VisuPar>            _vpu;         //!< For each partition: number of samples, index- start and end in u-knot-vector
      mutable DVector<VisuPar>            _vpv;         //!< For each partition: number of samples, index- start and end in v-knot-vector
      mutable DMatrix<CpIndex>            _cp_index;    //!< Indices of sample points affected by the control points

      // Selectors and selector grid
      bool                       _selectors;        // Mark if we have selectors or not
      bool                       _grid;             // Mark if we have a selector grid
  ...
  private:
      ...
      void                       resample( DMatrix<DMatrix <Vector<T,3> > >& p, const PreBasis<T>& bu, const PreBasis<T>& bv, int m1, int m2, int d1, int d2 ) const;
      ...
      void                       preSample();
      void                       preSample( PreBasis<T>& p, const DVector<T>& t, int m, int d, int n, T start, T end );
      void                       updateSamples() const;
      void                       makePartition();

      void                       comp(DMatrix<Vector<T,3>>& p, const DMatrix<T>& Bu, const DMatrix<T>& Bv, const Vector<T,3>& c, int k1, int k2) const;
