...
namespace GMlib {
  ...
  /*! PERBSSurf<T>::PERBSSurf( PSurf<T>* g, int no_locals_u, int no_locals_v )
   *  \brief Constructor copy surface of g, using SubSurfaces of g.
   *
   *  Making an ordinary blending-spline surface (ERBS)
   *
   *  \param[in]  g             The surface to copy.
   *  \param[in]  no_locals_u   Number of local patches in the u-direction.
   *  \param[in]  no_locals_v   Number of local patches in the v-direction.
   */
//  template <typename T>
//  inline
//  PERBSSurf<T>::PERBSSurf( PSurf<T,3>* g, int no_locals_u, int no_locals_v) {

//    _type_local = SUB_SURF;
//    g->setDerived(this);

//    _origin     = g;
//    init(g->isClosedU(), g->isClosedV());
...
//            _c[i][j] = new PBezierSurf<T>( g->evaluateParent( _u[i+1], _v[j+1], d1, d2),
//                                                              _u[i], _u[i+1], _u[i+2],
//                                                              _v[j], _v[j+1], _v[j+2]);
//            insertPatch( _c[i][j] );
//        }
...
//  }
  ...
  PERBSSurf<T>::PERBSSurf( const PERBSSurf<T>& copy ) : PSurf<T,3>( copy ) {
    ...
    init(copy._closed_u, copy._closed_v);
  ...
  void PERBSSurf<T>::edit( SceneObject* obj ) {
    ...
    if( bezier )
      bezier->updateCoeffs( _c[i][j]->getPos() - _c[i][j]->evaluateParent( _u[i+1], _v[j+1], 0, 0 )[0][0] );
...
//          if(this->isClosedU() && u > _u[_u.getDim()-3]) u -= getParDeltaU();
//          if(this->isClosedV() && v > _v[_v.getDim()-3]) v -= (_v[_v.getDim()-2] - _v[1]);

//std::cout << "uk=" << uk << "(u,v)= (" << u << ", " << v << ") \t" ;
//std::cout << "vk=" << vk << "(_u[uk], _v[vk])= (" << _u[uk] << ", " << _v[vk] << ") \t" ;

//if(u == _u[4]) {std::cout << "\n****u= " << u << std::endl;}// u -= getParDeltaU();}
...
//std::cout << " s0= " << s0[0][0] << ", ";;
          ...
          if( std::abs(v - _v(vk)) < 1e-5 ) {
              this->_p = s0;
//std::cout << "p= " << this->_p[0][0] << "  (knot)" << std::endl;
              return;
          }
          else {    // Blend Patches
...
//std::cout << "s1= " << s1[0][0] << "  \t";
              ...
              this->_p = s1;
//std::cout << "p= " << this->_p[0][0] << std::endl;
          }
      }
...
//    }
  ...
  template <typename T>
  inline
  void PERBSSurf<T>::generateKnotVector( DVector<T>& t, const T sp, const T ep, int n, bool closed ) {

    if(closed) {
        const T  dt = (ep-sp)/n;      // n- the number of intervalls (one extra between the last and the first)
        t.setDim(n+3);
        for(int i=0; i<n+3; i++ )
            t[i] = sp + (i-1)*dt;     // Equal distance between all knots
    }
    else {
        const T  dt = (ep-sp)/(n-1);  // (n-1)- is the number of intervalls
        t.setDim(n+2);
        t[0] = t[1] = sp;             // Set the start knots
        for(int i = 2; i < n; i++)
            t[i] = sp + (i-1)*dt;     // Set the "step"-knots - equal distance
        t[n] = t[n+1] = ep;           // Set the end knots
    }
  }
  ...
  void PERBSSurf<T>::generateKnotVector( PSurf<T,3>* g ) {
    ...
    generateKnotVector( _u, g->getParStartU(), g->getParEndU(), _c.getDim1(), g->isClosedU() );

    // Knot Vector in V direction
    generateKnotVector( _v, g->getParStartV(), g->getParEndV(), _c.getDim2(), g->isClosedV() );
  }
  ...
  void PERBSSurf<T>::generateKnotVector( PSurf<T,3>* g, T u_s, T u_e, T v_s, T v_e ) {
    ...
    // Knot Vector in U direction
    generateKnotVector( _u, u_s, u_e, _c.getDim1(), g->isClosedU() );

    // Knot Vector in V direction
    generateKnotVector( _v, v_s, v_e, _c.getDim2(), g->isClosedV() );
  }
  ...
  DMatrix< Vector<T,3> > PERBSSurf<T>::getC( T u, T v, int uk, int vk, T du, T dv ) const {
      ...
      if(this->_resample) {
          ...
          if(this->isClosedV() && cv == _v.getDim()-3) v -= this->getParDeltaV();

          T dm = 0;
          if(this->isClosedU() && u == _u[_u.getDim()-2]) dm = this->getParDeltaU();

          // Evaluate First local patch
          DMatrix< Vector<T,3> > c0 = _c(cu)(cv)->evaluateParent(u-dm, v, du, dv);
          // If on a interpolation point return only first patch evaluation
          if( std::abs(u - _u(uk)) < 1e-5 ) {
//std::cout << " knot-value: " << u << std::endl;
              return c0;
          }
          // Select next local patch in u direction

          if(this->isClosedU() && cu == _u.getDim()-4) u -= this->getParDeltaU();

          // Evaluate Second local patch
          DMatrix< Vector<T,3> > c1 = _c(cu+1)(cv)->evaluateParent(u, v, du, dv);
  ...
  void PERBSSurf<T>::insertPatch( PSurf<T,3>* patch ) {
    ...
    patch->setLocal(true);
    patch->setLighted( true );
    patch->setMaterial( GMmaterial::ruby() );
...
//  }
  ...
  void PERBSSurf<T>::showLocalPatches() {
    ...
    for( int i = 0; i < _c.getDim1()-1; i++ ) {
      for( int j = 0; j < _c.getDim2()-1; j++ ) {
  ...
  template <typename T>
  void PERBSSurf<T>::sample(int m1, int m2, int d1, int d2) {


   this->initSample(m1, m2, d1, d2);
    ...
    for( int i = 0; i < _pvi.getDim1(); ++i ) {
      for( int j = 0; j < _pvi.getDim2(); ++j ) {
        ...
        this->resample( p, m1, m2, d1, d2, seg_u(0), seg_v(0), seg_u(1), seg_v(1) );
...
/*! void  PERBSSurf<T>::init(bool closed_u, bool closed_v)
 *  \brief Private, not for public use
 *
 *  Default initiation for the constructors
 *
 *  \param[in]  closed_u   whether the surface is closed in u-direction or not
 *  \param[in]  closed_v   whether the surface is closed in v-direction or not
 */
  template <typename T>
  inline
  void PERBSSurf<T>::init(bool closed_u, bool closed_v) {
