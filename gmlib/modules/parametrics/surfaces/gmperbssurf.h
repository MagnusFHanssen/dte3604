...
namespace GMlib {
  ...
  class PERBSSurf : public PSurf<T,3> {
    ...
    struct PreBas  {
        ...
        Vector<int,2> ind;   //!< The index of the two local patches to compute
    };
  ...
  public:
    ...
    PERBSSurf(); // Dummy
//    PERBSSurf( PSurf<T,3>* g, int no_locals_u, int no_locals_v );
...
//    void                                preSample( int m1, int m2, int d1, int d2, T s_u, T s_v, T e_u, T e_v ) override;
    void                                sample(int m1 = 0, int m2 = 0, int d1 = 0, int d2 = 0) override;

  protected:

    SURF_TYPE                           _type_local;
    PSurf<T,3>*                         _origin;     //!< Optional - Original surface (to copy)
  ...
  private:
    ...
    void                                init(bool closed_u, bool closed_v);
