...
//*****************************************
  ...
  PTorus<T>::PTorus( T wheelrad, T tuberad1, T tuberad2 ) {
    ...
    init(wheelrad,tuberad1, tuberad2);
  ...
  template <typename T>
  void PTorus<T>::computeSurroundingSphere( const DMatrix<DMatrix<Vector<T,3>>>& /*p*/, Sphere<T,3>& s ) const {

      s.resetPos(Point<T,3>(T(0)));
      s.resetRadius(_max_r);
  }
  ...
  template <typename T>
  void PTorus<T>::init(T wheelrad, T tuberad1, T tuberad2) {

    _a = wheelrad;
    _b = tuberad1;
    _c = tuberad2;
    this->_dm = GM_DERIVATION_EXPLICIT;
    _max_r    = computeMaxRadius();
  }


  template <typename T>
  T PTorus<T>::computeMaxRadius() {
