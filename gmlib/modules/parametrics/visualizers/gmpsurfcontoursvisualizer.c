...
namespace GMlib {
  ...
  PSurfContoursVisualizer<T,n>::PSurfContoursVisualizer() {
  ...
  template <typename T, int n>
  inline
  void PSurfContoursVisualizer<T,n>::display() {
  ...
  template <typename T, int n>
  inline
  Color PSurfContoursVisualizer<T,n>::getColor( T d ) {

    // Find Index
    int idx;
    idx = d * ( _colors.size()-1 );
    if( idx == _colors.size()-1 ) idx--;
    if( (idx < 0) || (idx > _colors.size()-1) ) idx = 0;
  ...
  template <typename T, int n>
  inline
  Color PSurfContoursVisualizer<T,n>::getColorInterpolated( T d ) {
    ...
    double local_d = (double( _colors.size()-1 ) * d) - idx;
  ...
  template <typename T, int n>
  const std::vector<Color>& PSurfContoursVisualizer<T,n>::getColors() const {
  ...
  template <typename T, int n>
  inline
  T PSurfContoursVisualizer<T,n>::getCurvatureGauss( DMatrix< Vector<T,3> >& p ) {
  ...
  template <typename T, int n>
  inline
  T PSurfContoursVisualizer<T,n>::getCurvatureMean( DMatrix< Vector<T,3> >& p ) {
  ...
  template <typename T, int n>
  inline
  T PSurfContoursVisualizer<T,n>::getCurvaturePrincipalMax( DMatrix< Vector<T,3> >& p ) {
  ...
  template <typename T, int n>
  inline
  T PSurfContoursVisualizer<T,n>::getCurvaturePrincipalMin( DMatrix< Vector<T,3> >& p ) {
  ...
  template <typename T, int n>
  GM_PSURF_CONTOURSVISUALIZER_INTERPOLATION_METHOD PSurfContoursVisualizer<T,n>::getInterpolationMethod() const {

    return _method;
  }

  template <typename T, int n>
  GM_PSURF_CONTOURSVISUALIZER_MAP PSurfContoursVisualizer<T,n>::getMapping() const {

    return _mapping;
  }

  template <typename T, int n>
  void PSurfContoursVisualizer<T,n>::update() {

    DMatrix< DMatrix< Vector<T, n>>>& p = *(this->_p);
    DMatrix<Vector<float,n>>&   normals = *(this->_n);
  ...
  template <typename T, int n>
  inline
  T PSurfContoursVisualizer<T,n>::getValue( DMatrix< DMatrix< Vector<T, 3> > >& p, int i, int j ) {
  ...
  template <typename T, int n>
  void PSurfContoursVisualizer<T,n>::setColors( const std::vector<Color>& c ) {
    _colors = c;
  }
  ...
  template <typename T, int n>
  void PSurfContoursVisualizer<T,n>::setInterpolationMethod( GM_PSURF_CONTOURSVISUALIZER_INTERPOLATION_METHOD method ) {
    _method = method;
  }
  ...
  template <typename T, int n>
  void PSurfContoursVisualizer<T,n>::setMapping( GM_PSURF_CONTOURSVISUALIZER_MAP mapping ) {
    _mapping = mapping;
  }
