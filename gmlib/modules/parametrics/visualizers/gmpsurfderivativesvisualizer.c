...
namespace GMlib {
  ...
  PSurfDerivativesVisualizer<T,n>::PSurfDerivativesVisualizer(int u, int v)
    : _no_elements(0),_color( GMcolor::green() ), _size(1.0), _mode(GM_SURF_DERIVATIVESVISUALIZER_RELATIVE), _u(u), _v(v) {
  ...
  template <typename T, int n>
  PSurfDerivativesVisualizer<T,n>::PSurfDerivativesVisualizer(const PSurfDerivativesVisualizer<T,n>& copy)
      : PSurfVisualizer<T,n>(copy),
        _no_elements(0), _color(copy._color), _size(copy._size), _mode(copy._mode), _u(copy._u), _v(copy._v) {
  ...
  template <typename T, int n>
  inline
  void PSurfDerivativesVisualizer<T,n>::update() {
