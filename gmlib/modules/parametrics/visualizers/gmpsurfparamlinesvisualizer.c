...
PSurfParamLinesVisualizer<T,n>::PSurfParamLinesVisualizer(int nu, int nv)
  : _no_strips(0), _no_strip_indices(0), _strip_size(0),
    _mat(GMmaterial::blackRubber()), _n_u(nu), _n_v(nv) {
...
PSurfParamLinesVisualizer<T,n>::PSurfParamLinesVisualizer(const PSurfParamLinesVisualizer<T,n>& copy)
  : _no_strips(0), _no_strip_indices(0), _strip_size(0),
    _mat(copy._mat), _n_u(copy._n_u), _n_v(copy._n_v)  {
...
template <typename T, int n>
void PSurfParamLinesVisualizer<T,n>::setNumberLines(int nu=20, int nv=20) {
  _n_u = nu;
  _n_v = nv;
}
...
template <typename T, int n>
void PSurfParamLinesVisualizer<T,n>::update() {
  ...
  PSurfVisualizer<T,n>::fillNMap( _nmap, normals, this->_closed[0], this->_closed[1] );

  generatePTex( _n_u, _n_v, _n_u/2, _n_v/2, this->_closed[0], this->_closed[1] );
}
