...
//  }
  ...
  /*! bool  SceneObject::showSurroundingSphere(bool clean)
   *  \brief Pending Documentation
   *
   *  Pending Documentation
   */
  void  SceneObject::toggleDisplaySurroundingSphere()  {

      if(_sphere.isValid()) {
          if(!_sphere_vis)
              _sphere_vis = new SurroundingSphereVisualizer();

          if(!_visualizers.exist(_sphere_vis)) {
              _sphere_vis->setSphere(_sphere);
              SceneObject::insertVisualizer(_sphere_vis);
          }
          else
              SceneObject::removeVisualizer(_sphere_vis);
      }
  }
  ...
  void SceneObject::move( double d, bool propagate ) {
  ...
  void SceneObject::move(char c, double d, bool propagate) {
    ...
    if(_locked)
    {
      ...
      if(_dir_length<0)
         _dir_length = double((lock_pos - _pos).getLength());
    }
    ...
    if(_locked)
    {
       ...
       _pos  = lock_pos - _dir_length * _dir;
    } else
       _dir_length = -1;
  ...
  void SceneObject::move(const Vector<float,2>& t, bool propagate) {
    ...
    if(_locked)
    {
      ...
      // Trying to avoid unwanted jumps
      if(_dir_length<0)
         _dir_length = double((lock_pos - _pos).getLength());
  ...
  void SceneObject::rotateParent(Angle a, const Vector<float,3>& rot_axel, bool propagate) {
    ...
    if(_locked)
    {
      ...
      t = ra^_dir;
      t.setLength(float(tan(dd*M_PI_4)*dir_length));
      move(Vector<float,2>(t*_side,t*_up));
    }
    else
    {
