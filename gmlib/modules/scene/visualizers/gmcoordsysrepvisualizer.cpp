...
namespace GMlib {
  ...
  void CoordSysRepVisualizer::render(const SceneObject* /*obj*/, const DefaultRenderer *renderer) const {
    ...
    // Compute the position of the "coordinate-system visualization" relevant to the camera position
    float hh = -1.5f*cam->getNearPlane()*cam->getAngleTan();
    Point<float,3> cp(cam->getAspectRatio()*hh, hh, -(1+cam->getNearPlane()));
