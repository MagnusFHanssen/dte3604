...
// local
#include "gmpathtrackarrowsvisualizer.h"
...
PathTrackArrowsVisualizer::PathTrackArrowsVisualizer(Color color) : _color(color), _line_width(1.0f) {
...
void PathTrackArrowsVisualizer::render(const SceneObject *obj, const DefaultRenderer *renderer) const {
    ...
    _prog.bind(); {
        ...
        _vbo.bind();
          _vbo.enable(vert_loc, 3, GL_FLOAT, GL_FALSE, sizeof(GL::GLVertex), reinterpret_cast<const GLvoid*>(0x0));
            glDrawArrays( GL_LINES, 0, _no_vertices );
...
void PathTrackArrowsVisualizer::update() {

    _no_vertices = ((*_path_full) ? static_cast<int>((*_arrow_path).size()) : static_cast<int>(*_element));
