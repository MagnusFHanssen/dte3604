...
namespace GMlib {
  ...
  template <typename T>
  void TriangleFacets<T>::insertLine( const TSLine<T>& pwl ) {
  ...
  template <typename T>
  bool TriangleFacets<T>::insertVertex( const TSVertex<T>& v, bool c ) {
 ...
 // #endif
  ...
  template <typename T>
  bool TriangleFacets<T>::setConstEdge( const TSVertex<T>& v1, const TSVertex<T>& v2 ) {
